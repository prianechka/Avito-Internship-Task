...
// TestGetUserReportSuccess показывает, что программа способна сделать отчёт по пользователю без опциональных параметров.
func TestGetUserReportWithoutParamsSuccess(t *testing.T) {
	var (
		userID    int     = 1
		serviceID int     = 1
		sum       float64 = 100
		balance           = 100
		comment           = "Хорошо"
		orderBy           = utils.EmptyString
		limit             = utils.NotInQuery
		offset            = utils.NotInQuery
	)
