...
- Логирование запросов и ответов на них.
- Использование принципов чистой архитектуры при проектировании и реализации сервиса.
- Всё API имеет префикс /api/v1
- Реализован middleware, который используется для логирования и отслеживания возможной паники во время обработки запросов.

## Архитектура программы
Архитектура декомпозирована на пять слоёв:
1. Repo - репозитории, которые работают непосредственно с СУБД и возвращают ответы из базы.
2. Controllers - содержат узкую бизнес-логику (узкую в плане узкой предметной области), управляют данными, которые хранит repo.
3. Managers - управляют всеми контроллерами, содержат бизнес-логику в масштабе сценария или одного use-case случая.
4. Handlers - обрабатывают запросы, считывают аргументы с запроса, отправляют задачу менеджерам и отправляют ответы обратно.
5. Server - реализация непосредственно сервера: маршрутизация, подсоединение middleware обработчиков.

Для реализации каждая сущность хранит интерфейсы сущностей уровнем ниже (Controllers в структуре имеет RepoInterface), таким образом, поддерживается возможность нескольких реализаций одной и той же сущности -> расширяемость программы.
